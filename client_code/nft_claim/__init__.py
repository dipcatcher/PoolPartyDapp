...
class nft_claim(nft_claimTemplate):
  def __init__(self, **properties):
    ...
    if get_open_form().metamask.address is not None:
      ...
      self.label_content.text = "If you claimed names during the Nametag Claim phase, connect your wallet to mint them as NFTs which can be utilized for your pools or to list on NFT marketplaces."
      self.data_grid_1.visible = False

    # Any code you write here will run before the form opens.
  def collect_users_names(self):
    all_claim_events =anvil.js.await_promise(get_open_form().get_contract_read("NAMECLAIM").queryFilter('Claim', 0, "latest"))
    claimed_list = []
    all_claimed_list = []
    ...
    if len(claimed_list) ==0:
      self.label_content.text = "No nametags claimed by this address during the Ticker Name Claim phase found."
      self.data_grid_1.visible = False
